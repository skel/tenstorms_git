    ...
    public:
        ...
        class spell_lich_king_infection_AuraScript : public AuraScript
        {
            ...
            void OnPeriodic(AuraEffect const* aurEff)
            {
                ...
                if(GetTargetApplication()->GetTarget()->GetHealthPct() < 90 || !GetTargetApplication()->GetBase()->GetCaster())
                    return;

                GetTargetApplication()->GetTarget()->RemoveAurasDueToSpell(SPELL_INFEST);
            }
...
class spell_lich_king_quake : public SpellScriptLoader
{
    public:
        ...
        class spell_lich_king_quake_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                if(!GetTargetApplication()->GetBase()->GetCaster() || !GetTargetApplication()->GetBase()->GetOwner())
                    return;

                GetTargetApplication()->GetBase()->GetCaster()->CastSpell(GetTargetApplication()->GetBase()->GetCaster(), SPELL_WMO_DAMAGE, true);
                if (InstanceScript* instance = GetTarget()->GetInstanceScript())
                    if (Creature* lichKing = Unit::GetCreature(*GetTargetApplication()->GetTarget(), instance->GetData64(DATA_LICH_KING)))
